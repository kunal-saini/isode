...
const start_options = {};
const bindPath =  path.join(__dirname, 'volume') + ':/isode:ro';
...
function create (o) {
    ...
    const create_container_options = {
        AttachStdout: true,
        AttachStderr: true,
        Tty: true,
    ...
    const docker = new Docker({socketPath: options.socket_path});
    const containers = {};
    ...
    return {
        run: function (isolate_key, code, cb) {
            ...
            const container = containers[isolate_key];
            if (Array.isArray(container)) {
                ...
                containers[isolate_key] = [{code: code, cb: cb}];
                createContainer();
            }
            else {
            ...
            function createContainer() {
                ...
                async.series([
                    ...
                    function (cb) {
                        // Register container
                        const requests = containers[isolate_key];
                        const container = containers[isolate_key] = {
                        ...
                        // [temp fix] wait for the docker http server to start
                        setTimeout(function () {
